...
def chance_of_mutation(child, quantity_before_stopping):
...
def make_child(parent1, parent2, quantity_before_stopping):
    ...
    # for _ in range(random.randint(1, number_of_cities - 1)):
    child = chance_of_mutation(child, quantity_before_stopping)
...
def select_one(population, cumulative):
    ...
    for i in range(population_size):
        if chance <= cumulative[i]:
            if population[i] != [fitness(30000), 30000, [0] * number_of_cities]:
                return population[i]
    return population[-1]
...
def main():
    ...
    for raw in roads_weight:
        print(raw)

    population = [[fitness(30000), 30000, [0] * number_of_cities] for _ in range(population_size)]
    for i in range(population_size):
        ...
        if child not in population:
            population[i] = child
        # population.append([fitness(distance), distance, random_cities])
    ...
    best_population = population[population_size - 1]

    # results.append(f"{best_population[0]}")
    results.append(f"{best_population[1]}")
